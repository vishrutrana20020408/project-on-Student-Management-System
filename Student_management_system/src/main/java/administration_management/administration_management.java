...
//-----------------------------------------------------------------------ADMIN LOGIN-------------------------------------------------------------
	...
	private static void addTeacher() {
	    ...
	    try (Connection conn = UserDAO.getConnection()) {
	        ...
	        String checkQuery = "SELECT username FROM teachers WHERE username = ?";
	        PreparedStatement checkStmt = conn.prepareStatement(checkQuery);
	        checkStmt.setString(1, username);
	        ResultSet rs = checkStmt.executeQuery();
	        ...
	        // Insert new teacher
	        String insertQuery = "INSERT INTO teachers (name, email, phone, course, username, password) VALUES (?, ?, ?, ?, ?, ?)";
	        PreparedStatement insertStmt = conn.prepareStatement(insertQuery);
	        insertStmt.setString(1, name);
	        insertStmt.setString(2, email);
	        insertStmt.setString(3, phone);
	...
	private static void removeTeacher() {
	    ...
	    try (Connection conn = UserDAO.getConnection()) {
	        // Show all teachers
	        String showQuery = "SELECT name, username, email, phone, course FROM teachers";
	        Statement stmt = conn.createStatement();
	        ResultSet rs = stmt.executeQuery(showQuery);
	        ...
	        String checkQuery = "SELECT * FROM teachers WHERE username = ? AND name = ? AND course = ?";
	        PreparedStatement checkStmt = conn.prepareStatement(checkQuery);
	        checkStmt.setString(1, username);
	        checkStmt.setString(2, name);
	        ...
	        if (confirm.equals("Y")) {
	            String deleteQuery = "DELETE FROM teachers WHERE username = ? AND name = ? AND course = ?";
	            PreparedStatement deleteStmt = conn.prepareStatement(deleteQuery);
	            deleteStmt.setString(1, username);
	            deleteStmt.setString(2, name);
...
// ------------------------------------------------------------------------END OF LOGIN ADMIN----------------------------------------------------
	...
	private static void loginTemporaryStudent() {
        try {
            ...
            Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASS);
            String query = "SELECT * FROM temporary_students WHERE username = ? AND password = ?";
            PreparedStatement pstmt = conn.prepareStatement(query);
            pstmt.setString(1, username);
            pstmt.setString(2, hashedPassword);
